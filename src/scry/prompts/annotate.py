...
# wholes; the links and texts paragraphs are the ids paragraphs with a point for a box id and their every rule kept;
# CONTAINERS, NO_TEXTS and DESCRIPTION are shared. The point is described here once and once in the schema.
IMAGES_COORDS = """You are shown one screenshot. An OCR engine has detected the pieces of text on it, and the user message lists every box it detected as a rectangle, x0,y0,x1,y1, in the coordinates it states; it names the targets, the boxes you are asked to label, as rectangles too. Wherever an answer points at a box, give a point: the x and y of a pixel inside that box, near the middle of its text, in the same coordinates. Code matches each point to the box that contains it."""

ASSIGN_COORDS = """assign: for every target, a point inside it and the container it belongs to. Every target appears exactly once, or in unassigned."""

LINKS_COORDS = """links: relations between boxes of one container, given as three lists; every box in them is named by a point inside it. A link must include at least one target; it may also include boxes that are not targets. A box may appear in at most one link: a box that is in a run, a pair or a record appears in no other link of any of the three lists; column headings are the exception and may be named by every record of their table.
  runs: boxes that are one continuous piece of text which the OCR engine split or the screen wrapped onto the next line, in reading order, one point each. joiner is "" when a word was cut in two by the wrap, " " otherwise. Separate lines of terminal output, separate rows of a list or table and separate menu items are NOT a run, even when they follow one another: a long file path that reached the edge of its window and continued on the line below is one run; the command make build on one line and the command make test on the line below are two separate texts, not a run.
  pairs: a label and its value (a property and its value, a form field and its content). key and value are lists of points and neither is empty: a pair needs at least two different boxes, the label in one and its value in another. When one box already holds both the label and its value (a single box that reads "Language: English"), give no link for it. When a label or a value that wrapped onto two boxes is one side of a pair, list its boxes together, in reading order, inside that pair's key or value and give no separate run for them: a label on two boxes whose value is a third box is the one pair with the two points in key and the third in value. A two-column grid of labels and values is pairs, not records.
...
TEXTS_COORDS = """texts: for every target, a point inside it and the verbatim text inside that box, read from the screenshot. Preserve case, punctuation, whitespace and symbols. Never correct, complete or normalize commands, code, paths or identifiers. Use ? for a character you cannot resolve. An icon is not text: give "". missed: text no box covers, with its container."""
