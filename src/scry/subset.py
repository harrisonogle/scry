...
def make_subset(src: Run, out: Path, frames: tuple[int, int], share_cache: bool = True) -> Run:
    """Derive a run directory holding only the decoded frames lo..hi of `src`, with decode marked done in its manifest so
    ...
    lo, hi = frames
    recs = [r for r in src.load_frames() if lo <= r.frame <= hi]
    if not recs:
        raise ValueError(f"no frames in {lo}-{hi} under {src.root}")
    m = src.manifest_read()
    st = m.get("stages", {}).get("decode")
    if st is None:
        raise ValueError(f"{src.root} has no finished decode stage in its manifest")
    out = Path(out)
