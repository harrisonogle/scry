...
const ORANGE = new Set(['198:b89', 'b89', 'L1611']);
...
const markCites = (line: SChar[]): SChar[] => {
...
const parseMarkdown = (line: string): SChar[] => {
...
const weightOf = (b: Block) => (b.kind === 'cmd' ? 0.1 : b.kind === 'note' ? 0.05 : b.kind === 'call' ? 0.07 : 0.25);
export const HOLD = 0.18;

const rowsFor = (blocks: Block[], p: number): {rows: Row[]; cursor: boolean} => {
  const rows: Row[] = [];
  let cursor = false;
  let t0 = 0;
    ...
    if (block.kind === 'cmd') {
      ...
      block.results.forEach((r, j) => {
        if (local >= 0.45 + j * 0.2 || done) {
          const body = wrap(markCites(chars(r, {dim: true})), 0, COLS - 5);
...
type Props = {title: string; status: string; blocks: Block[]; caption: string};

export const TerminalScene: React.FC<Props> = ({title, status, blocks, caption}) => {
  const frame = useCurrentFrame();
  const {durationInFrames} = useVideoConfig();
  // The segments sum to 1 - HOLD, so the finished transcript holds for the last HOLD of the scene.
  const p = interpolate(frame, [FADE_FRAMES, durationInFrames - FADE_FRAMES], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const {rows, cursor} = rowsFor(blocks, p);
