"""P14 label quality: box_text against plain, yardstick = both P10 ids100 repeats (the same call as plain). Usage: qual.py smoke"""
...
class R:
    def __init__(self, name):
        ...
        m = json.loads((root / name / 'manifest.json').read_text())['stages']['annotate']
        u = m['usage']; self.calls = m['calls']; self.frames = m['frames']; self.cost = m['cost_usd']; self.version = m['prompt_version']
        self.tok_in = (u['input_tokens'] + u['cache_read_input_tokens'] + u['cache_creation_input_tokens']) / max(1, self.calls)
        self.tok_out = u['output_tokens'] / max(1, self.calls)
...
print('\nCOST  run | version | calls | $/frame | input tok/call (incl. cache) | output tok/call')
for n, x in runs.items():
    print(f"{n} | {x.version} | {x.calls} | {x.cost / x.frames:.4f} | {x.tok_in:.0f} | {x.tok_out:.0f}")
