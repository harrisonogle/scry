...
print('run | pair links | off-row pairs (dy>0: no shared row) | frames | runs with a jump (dy>12 or dx>300 between consecutive boxes)')
for span in ('smoke','span2'):
    for s in ['s100','sonnet5']:
        for r in (1,2):
            n=f"{'p7' if s=='sonnet5' else 'p3'}/{span}-{s}-r{r}"
            ann,boxes=load(n)
            tot=0; off=[]; rj=[]
            for f,a in ann.items():
                ...
                for l in a['links']:
                    if l['kind']=='pair':
                        ...
                        if g[1]>0: off.append((f,g,' + '.join(bb[k]['text'][:25] for k in l['key']),'=>',' + '.join(bb[v]['text'][:25] for v in l['value'])))
                    elif l['kind']=='run':
