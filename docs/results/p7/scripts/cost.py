...
for phase,scales in SETS:
    ...
    for span in ['smoke','span2']:
        ...
        for s in scales:
            ...
            res[(phase,span,'x')]=dict(incall=incall,outcall=outcall,cpf=cpf,wall=wall,mm=mm,
...
print('\n== SIDE BY SIDE (mean of repeats; smoke | span2): D=Sonnet5 (p7), A=Opus5 s100 (p3), D-A')
for sc in ['x']:
    for key,fmt in [('incall','{:.0f}'),('outcall','{:.0f}'),('cpf','{:.4f}'),('wall','{:.0f}'),('mm','{:.1f}')]:
        cells=[]
        for span in ['smoke','span2']:
            d=res.get(('p7',span,sc)); a=res.get(('p3',span,sc))
