"""P10 label quality (group-only measures), local runs beside the ids100 yardstick. Usage: qual_p10_local.py <span> <run-dir>..."""
...
class R:
    def __init__(self, name, path=None):
        self.name = name; path = path or ROOT / name; self.run = Run(path)
...
# --- local2 additions: per-call tokens and validity from the run's call cache; frame 147's answer ---
print('\nCALLS (from the run cache): run | calls | valid JSON | prompt tokens/call (mean, min-max) | completion tokens/call (mean, min-max) | frame 147: assign entries, empty box ids, unplaced, unknown_box')
for n, x in runs.items():
    cdir = x.run.root / 'cache'
    ...
    ok = sum(1 for c in calls if not c['response'].get('error'))
    pt = [c['response']['usage'].get('input_tokens', 0) for c in calls]; ct = [c['response']['usage'].get('output_tokens', 0) for c in calls]
    a147 = x.ann.get(147)
    f147 = '-'
    if a147 is not None:
        parsed = next((c['response'].get('parsed') for c in calls if (c['response'].get('parsed') or {}).get('description') == a147.description), None) or {}
        empty = sum(1 for e in parsed.get('assign', []) if not e.get('box'))
        rk = a147.repair_counts
        f147 = f"{len(parsed.get('assign', []))} assign, {empty} empty ids, unplaced {rk.get('unplaced', 0)}, unknown_box {rk.get('unknown_box', 0)}, containers {[(c.kind, c.name[:30]) for c in a147.containers]}"
    m = lambda v: f"{np.mean(v):.0f} ({min(v)}-{max(v)})" if v else '-'
    print(f"{n} | {len(calls)} | {ok} | {m(pt)} | {m(ct)} | {f147}")
