...
WT = Path('<repo>/.claude/worktrees/localvlm')
...
json.dump({n: {k: v for k, v in s.items()} for n, s in store.items()}, open(f'<scratch>/localvlm/qual-{span}-detail.json', 'w'), indent=0, default=str)
