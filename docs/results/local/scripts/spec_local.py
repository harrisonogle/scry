"""Specific checks, local runs beside the P3 s100 references. Usage: spec_local.py <span> <run-dir>..."""
import json, sys, re
...
def load(n, path=None):
    path = path or ROOT / n
    ann = {}
    for l in open(path / 'annotations.jsonl'):
        a = json.loads(l); ann[a['frame']] = a
    boxes = {}
    for l in open(path / 'boxes.jsonl'):
...
for r in (1, 2):
    n = f'{span}-s100-r{r}'
    if (ROOT / n / 'annotations.jsonl').exists(): runs[n] = load(n)
for p in EXTRA:
    if (p / 'annotations.jsonl').exists(): runs[p.parent.name + '/' + p.name] = load(p.name, p)
names = list(runs)
# app vocabulary
voc = Counter()
...
pat = re.compile(r"\bb\d{1,3}\b|\bbox(?:es)?\s+\d|\bnumbered\b|\boverlay\b|\bmagenta\b|\byellow (?:tag|label)|\btargets? (?:box|were|was|is|are|list)|no targets|\bannotat|image [12]\b", re.I)
for n, (ann, boxes) in runs.items():
