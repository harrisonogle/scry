...
- `stages/annotate/NN.png`: the labels in force at the frame as `scry.annotate.join.Labels` joins them (the view the index uses). One blue outline around every box of the frame assigned to any window (all windows collapsed, no tag; it usually reaches the image edge because the desktop and taskbar are windows too); each popup as its own orange outline, popups collapsed by kind and tagged "popup: toast", "popup: panel", "popup: dropdown" or "popup: tooltip" (the kind read from the model's container name, e.g. "Notification: Saved Traffic Manager profile changes" is a toast, "Add endpoint blade" a panel, "Azure search dropdown" a dropdown); every pair link in force as a thin aqua line from the key box's right edge to the value box's left edge; every run in force as a thin violet (#4a3aa7) bracket under its boxes. Record links (tables) are not drawn. Boxes with no label in force are untouched.
